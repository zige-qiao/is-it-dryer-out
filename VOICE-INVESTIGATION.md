...
## Current handoff — through v0.5.4.12

### iOS system recording stop and muted meter

The user tapped the iPhone's amber microphone indicator and confirmed the system prompt to stop audio recording during build `v0.5.4.11-foreground-voice` (asset 129). At 27.856s, the app logged `track mute state=live muted=true` for its original `getUserMedia` waveform stream; there was no `ended` event or subsequent `unmute` in the supplied log. The app then reused that same `live` but muted meter on sessions 4–10, explaining the flat waveform. Every one of those sessions still reached `speechstart` and produced interim and final recognition results. The user's observation that the amber indicator thereafter turned on and off with recognition agrees with a distinct working Web Speech path, but does not prove where WebKit sends audio. A later `audio-capture: Source is muted` error was logged for a stale, non-current recogniser after session 8 and did not prevent sessions 9–10 from transcribing.

Build `v0.5.4.12-muted-meter-fallback` (asset 130) treats a muted retained meter as unusable for waveform display, without automatically reopening capture or changing recognition startup. A mute during an active attempt pauses waveform drawing and switches to the existing meterless pulse. A later explicit voice attempt skips reuse of the still-muted meter, retains it only until the normal foreground cleanup, and starts recognition as before. If the track un-mutes during an active attempt, drawing may resume. This preserves the observed working transcription behaviour while avoiding a misleading flat waveform. It is a UI/lifecycle mitigation, not a verified way to restore the real waveform or WebKit's audio session. iPhone validation of this build remains pending.

### Product decision and .11 implementation

User explicitly accepted persistent amber microphone indication and requested keeping capture throughout the foreground app session, including Apply and dialog close, with no new Stop/Abort/Release buttons or “Microphone ready for retry” copy. Build `v0.5.4.11-foreground-voice`, app asset 128, implemented this iPhone policy. Retention no longer expires after 30 seconds; recognition's existing 30-second watchdog remains. Natural/manual completion and dialog cancellation detach the session from the enabled stream and cancel/reset its waveform; the next explicit attempt reuses an unmuted stream and restarts animation. Background/page exit releases capture. Invalid preparation still cleans up. This is an accepted UX/privacy trade-off, not a fix to Safari's underlying restart defect. The later asset-129 device run transcribed successfully across ten sessions, including after the retained meter became muted.
