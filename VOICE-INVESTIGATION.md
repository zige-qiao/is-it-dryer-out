...
**Status (2026-09-24): OPEN — releasing and reopening the microphone can silence both the waveform and transcription on the affected iPhone.** Retaining the stream is a verified limited mitigation, not a resolved privacy-compatible Stop/Abort lifecycle. Read the current handoff below before drawing conclusions from the historical sections. Earlier natural-completion and retained-stream successes remain valid observations, not proof of a platform fix.
...
Diagnostic branch `v0.5.4.13-muted-meter-reopen-test`, asset 131, tested a different recovery. After one successful session, the system muted the retained track at 19.255s. The user tapped the app microphone again at 29.787s. The app stopped the old track, requested a fresh `getUserMedia` stream, and opened a new track at 31.433s that reported `live`, `enabled=true`, `muted=false`, with a `running` AudioContext. Yet every one-second probe from 32.440s through 43.608s measured `levelMax=0.0000` while the user spoke, and recognition emitted no `speechstart` or results. Manual Stop ended with `aborted` / `No speech detected`. Sessions 3 and 4 reused that new stream, again measured zero throughout and produced no speech results. Thus explicit release/reopen made both the real waveform and transcription fail in this run, whereas the earlier retained-muted-stream run continued transcribing with a flat waveform. `live`, unmuted track state, a running context and advancing animation frames did not establish real microphone audio. The data locates the failure upstream of both consumers in this run but does not identify the exact WebKit or hardware stage. This branch was removed from the public Pages source; `main` retains asset 129. Do not merge this reopening strategy as a recovery.
