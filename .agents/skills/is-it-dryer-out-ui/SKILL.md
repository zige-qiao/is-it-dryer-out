...
Voice input, when supported by the browser, updates indoor temperature and relative humidity only. Keep a review step before applying values and allow immediate manual stop. Start speech recognition without waiting for the optional visual meter. On desktop, use an adaptive real microphone level for the waveform and one continuous second of silence; on iOS, let speech recognition own the microphone exclusively and use flat bars with a listening-outline pulse instead of opening a competing meter stream. Stop recognition before releasing its audio resources, isolate each recording session, and ensure stale callbacks cannot stop a newer session. Do not infer a single unlabelled integer when it is valid for both temperature and humidity. Keep temporary on-device diagnostics behind an explicit query flag and avoid logging recognised speech content.
