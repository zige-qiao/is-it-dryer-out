...
Voice input, when supported by the browser, updates indoor temperature and relative humidity only. Keep a review step before applying values and allow immediate manual stop. On desktop, start speech recognition without waiting for the optional adaptive microphone meter. On iOS, open the standard microphone meter first and keep its stream alive throughout speech recognition; this preserves the platform audio session and provides the real waveform and one-second silence detection. Stop recognition before releasing meter resources, isolate each recording session, and ensure stale callbacks cannot stop a newer session. If the meter is unavailable, allow recognition to continue with the meterless listening-outline pulse. Do not infer a single unlabelled integer when it is valid for both temperature and humidity. Keep temporary on-device diagnostics behind an explicit query flag and avoid logging recognised speech content.
