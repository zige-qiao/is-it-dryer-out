...
Current voice-policy override (v0.5.4.12): on iOS, after explicit user voice activation retain the enabled stream for the entire foreground session, including completion, manual Stop, Apply, dialog close and recognition errors. Reset/cancel waveform animation outside active attempts and restart it when reusing an unmuted meter. If iOS mutes the retained track without ending it, do not reuse it for waveform drawing or automatically reopen capture; preserve the working recognition path and show the meterless listening pulse. No retained idle timeout, new buttons, release warning or readiness copy. Release on hidden/pagehide and clean up invalid resources. The user accepts the persistent amber dot; safe meter recovery and background-release recovery remain unresolved. This replaces the older natural-end/dialog-close/30-second release policy described historically below.
