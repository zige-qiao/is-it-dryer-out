...
On iOS, the live waveform stream is also an audio-session keep-alive. Earlier diagnostics removed that stream under the assumption that it competed with speech recognition; the first attempt then worked while immediate retries emitted `audiostart` without activating the physical microphone. A short microphone reset did not help, but keeping the normal stream open throughout recognition succeeded in three consecutive standalone attempts and three consecutive production-app attempts. The app therefore treats the held stream as required iOS lifecycle infrastructure: it opens before a fresh one-shot recogniser, remains active until recognition ends, drives the waveform without stopping recognition, and is then released.

The [iOS voice recognition investigation](VOICE-INVESTIGATION.md) contains the consolidated device-test record, event timings, regression history, verified natural-completion mitigation, reopened manual-stop question, and related WebKit reports. The supporting [video analysis](tests/ios_web_speech_microphone_video_analysis.md) documents the Safari and Chrome screen recording in detail.
