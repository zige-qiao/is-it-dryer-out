...
Current iPhone voice policy (`v0.5.4.12-muted-meter-fallback`, app asset 130): after the first user-requested voice attempt, keep the microphone stream enabled throughout the foreground session, including natural completion, manual Stop, Apply and dialog close. The amber indicator may remain on. Transcription ends separately; the waveform is reset and its animation cancelled between attempts. If iOS mutes the retained meter track without ending it, the app leaves the working speech-recognition path alone and shows the meterless listening pulse instead of a flat waveform. It does not automatically reopen capture in this state; restoring the waveform safely remains unverified on iPhone. No new controls or readiness message are added. Backgrounding or leaving releases capture; restarting after that release is still an unresolved platform risk. No audio file is saved or uploaded by the meter; browser speech recognition may use an external service. This supersedes the older 30-second/dialog-bounded retention description below.
